                ...
                {:else if message.type === 'status'}
                    <div class="status-content">
                        ...
                        {#if message.commands}
                            <div class="command-output">
                                <div class="command-section">
                                    <h4>Available Commands ({message.commands.length})</h4>
                                    <div class="commands-list">
                                        {#each message.commands as cmd}
                                ...
                                <div class="command-section">
                                    ...
                                    <div class="info-grid">
                                        <div class="info-item">
                                            ...
                                            <span class="info-value">{message.session_name}</span>
                                        </div>
                                        <div class="info-item">
                                            <span class="info-label">Model</span>
                                            <span class="info-value">{message.model}</span>
                                        </div>
                                        <div class="info-item">
                                            <span class="info-label">Role</span>
                                            <span class="info-value">{message.role}</span>
                                        </div>
                                        <div class="info-item">
                                            <span class="info-label">Tokens Used</span>
                                            <span class="info-value">{message.tokens_used?.toLocaleString() || 0}</span>
                                        </div>
                                        <div class="info-item">
                                            <span class="info-label">Total Cost</span>
                                            <span class="info-value">${message.total_cost?.toFixed(4) || '0.0000'}</span>
                                        </div>
                                        <div class="info-item">
                                            <span class="info-label">Cache Write</span>
                                            <span class="info-value">{message.tokens_cache_write?.toLocaleString() || 0} tokens</span>
                                        </div>
                                        <div class="info-item">
                                            <span class="info-label">Cached</span>
                                            <span class="info-value">{message.tokens_cached?.toLocaleString() || 0} tokens</span>
                                        </div>
                                        <div class="info-item">
                                            <span class="info-label">Cache Savings</span>
                                            <span class="info-value">${message.cache_savings?.toFixed(4) || '0.0000'}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        {#if message.command_type === 'report' && message.entries}
                            <div class="command-output">
                                <div class="command-section">
                                    <h4>Session Report</h4>
                                    <div class="report-entries">
                                        {#each message.entries as entry}
                                    ...
                                    {#if message.totals}
                                        <div class="report-totals">
                                            <div class="total-item">
                                                ...
                                                <span class="total-value">${message.totals.total_cost.toFixed(4)}</span>
                                            </div>
                                            <div class="total-item">
                                                <span class="total-label">Total AI Time</span>
                                                <span class="total-value">{(message.totals.total_ai_time_ms / 1000).toFixed(1)}s</span>
                                            </div>
                                            <div class="total-item">
                                                <span class="total-label">Total Human Time</span>
                                                <span class="total-value">{(message.totals.total_human_time_ms / 1000 / 60).toFixed(1)}m</span>
                                            </div>
                                            <div class="total-item">
                                                <span class="total-label">Tool Calls</span>
                                                <span class="total-value">{message.totals.total_tool_calls}</span>
                                            </div>
                                        </div>
                                    {/if}
                                </div>
                        ...
                        {#if message.command_type === 'plan'}
                            <div class="command-output">
                                <div class="command-section">
                                    {#if message.has_plan && message.plan}
                                        <h4>📋 {message.plan.plan_title}</h4>
                                        <div class="plan-progress">
                                            <span class="plan-progress-text">Task {message.plan.current_task}/{message.plan.total_tasks}</span>
                                            <div class="plan-progress-bar">
                                                <div class="plan-progress-fill" style="width: {(message.plan.current_task / message.plan.total_tasks * 100)}%"></div>
                                            </div>
                                        </div>
                                        <div class="plan-current-task">
                                            <div class="current-task-label">CURRENT TASK</div>
                                            <div class="current-task-title">{message.plan.current_task_title}</div>
                                            <div class="current-task-desc">{message.plan.current_task_description}</div>
                                        </div>
                                        <div class="plan-tasks">
                                            {#each message.plan.tasks as task, index}
                                    ...
                                    {:else}
                                        ...
                                        <div class="plan-no-plan">
                                            {@html formatMessage(message.display || 'No plan is currently active. Use plan tool for complex, multi-step tasks.')}
                                        </div>
                                    {/if}
                                </div>
                            </div>
                        ...
                        {#if message.command_type === 'context'}
                            <div class="command-output">
                                <div class="command-section">
                                    <h4>📋 Context ({message.total_messages || 0} messages)</h4>
                                    {#if message.filter}
                                        <div class="context-filter">
                                            <span class="filter-label">Filter:</span>
                                            <span class="filter-value">{message.filter}</span>
                                        </div>
                                    {/if}
                                    <div class="context-messages">
                                        {#each message.filtered_messages || [] as msg}
                        ...
                        {#if message.command_type === 'list'}
                            <div class="command-output">
                                <div class="command-section">
                                    <h4>📋 Available Sessions (Page {message.page || 1} of {message.total_pages || 1})</h4>
                                    <div class="list-summary">
                                        <span>Showing {message.sessions?.length || 0} of {message.total_sessions || 0} sessions</span>
                                    </div>
                                    {#if message.sessions && message.sessions.length > 0}
                                        <div class="sessions-table">
                                            ...
                                            {#each message.sessions as session}
                                        ...
                                        {#if message.total_pages > 1}
                                            <div class="list-navigation">
                                                <div class="nav-info">
                                                    ...
                                                    <span class="nav-hint">Next: <code>/list {message.page + 1}</code></span>
                                                    {#if message.page > 1}
                                                        <span class="nav-hint">Previous: <code>/list {message.page - 1}</code></span>
                                                    {/if}
                        ...
                        {#if message.command_type === 'role'}
                            <div class="command-output">
                                <div class="command-section">
                                    <h4>👤 Role Configuration</h4>
                                    <div class="role-current">
                                        <span class="role-label">Current Role:</span>
                                        <span class="role-value">{message.current_role}</span>
                                        {#if message.changed}
                                            <span class="role-changed">changed from {message.old_role || 'none'}</span>
                                        {/if}
                                    </div>
                                    {#if message.available_roles && message.available_roles.length > 0}
                                        <div class="role-list">
                                            <div class="role-list-header">Available Roles:</div>
                                            {#each message.available_roles as role}
                                                <div class="role-item {role === message.current_role ? 'active' : ''}">{role}</div>
                                            {/each}
                                        </div>
                                    {/if}
                                </div>
                        ...
                        {#if message.command_type === 'model'}
                            <div class="command-output">
                                <div class="command-section">
                                    <h4>🤖 Model Configuration</h4>
                                    <div class="model-current">
                                        <span class="model-label">Current Model:</span>
                                        <span class="model-value">{message.new_model}</span>
                                        {#if message.changed}
                                            <span class="model-changed">changed from {message.old_model || 'none'}</span>
                                        {/if}
                                    </div>
                                </div>
                            </div>
                        ...
                        {#if message.command_type === 'workflow'}
                            <div class="command-output">
                                <div class="command-section">
                                    <h4>⚙️ Workflows</h4>
                                    {#if message.data?.workflows && message.data.workflows.length > 0}
                                        <div class="workflow-list">
                                            {#each message.data.workflows as workflow}
                                                <div class="workflow-item">{workflow}</div>
                                            {/each}
                                        </div>
                                    {/if}
                                    {#if message.data?.message}
                                        <div class="workflow-message">{message.data.message}</div>
                                    {/if}
                                </div>
                            </div>
                        {/if}
